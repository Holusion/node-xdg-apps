...
class Finder{
  ...
  async findEntry (file) {
    ...
    console.log("finding mime in %dms", performance.now()-start);
    if(this.type == "desktop"){
      const default_apps = await find(mime_type);
      console.log("finding default app in %dms", performance.now()-start);
      const default_entries = await Promise.all(default_apps.map(app=>this.entries.getEntry(app)));
      console.log("finding default app entries in %dms", performance.now()-start);
    ...
    console.log("Finding entries in %dms", performance.now()-start);
    ...
    console.log("Returning in %dms", performance.now()- start);
